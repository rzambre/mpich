                                          ...
                                          int requests_property)
{
    ...
    if (requests_property & MPIR_REQUESTS_PROPERTY__NO_GREQUESTS) {
        ...
        int i;
        int n_completed;
        int num_reqs_in_vci[MPIDI_CH4_MAX_VCIS] = {0};
        int vci_of_req;
        ...
        for (i = 0; i < count; i++) {
            if (request_ptrs[i] == NULL) {
                ...
                if (!MPIR_Request_is_complete(request_ptrs[i])) {
                    ...
                    num_reqs_in_vci[vci_of_req]++;
                    
                    /* If condition ensures that distinct VCIs in the array of requests are poked at least once */
                    if ((num_reqs_in_vci[vci_of_req] - 1) % MPIR_CVAR_REQUEST_POLL_FREQ == 0) {
                        /* Per-VCI progress on this req */
                        mpi_errno = MPID_Progress_test_req(request_ptrs[i]);
                        if (mpi_errno)
                            MPIR_ERR_POP(mpi_errno);
