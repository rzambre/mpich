                                                     ...
                                                     ucp_tag_recv_info_t * info)
{
    ...
    if (ucp_request->req)
         ...
         * User needs to store VCI in the tag. Will only work
         * if sender and receiver use the same VCI.
         * TODO: this is way too hacky */
        int vci;
        vci = MPIDI_UCX_get_tag(info->sender_tag); 
                                            ...
                                            int hst_vci, int rmt_vci)
{
    ...
    if (dt_contig) {
        ucp_request =
            (MPIDI_UCX_ucp_request_t *) ucp_tag_recv_nb(MPIDI_UCX_VNI(hst_vni).worker,
    ...
    if (ucp_request->req) {
        ...
        if (req == NULL)
            req = MPID_Request_create_unsafe(MPIR_REQUEST_KIND__RECV, hst_vci);
