...
#ifdef MPIDI_CH4_DIRECT_NETMOD
    mpi_errno =
        MPIDI_NM_mpi_irecv(buf, count, datatype, rank, tag, comm, context_offset, av, request, hst_vci, rmt_vci);
#else
    if (unlikely(rank == MPI_ANY_SOURCE)) {
        mpi_errno =
            MPIDI_SHM_mpi_irecv(buf, count, datatype, rank, tag, comm, context_offset, request);
...
MPL_STATIC_INLINE_PREFIX int MPIDI_recv_safe(void *buf,
                                             ...
                                             MPIR_Comm * comm,
                                             int context_offset, MPIDI_av_entry_t * av,
                                             MPI_Status * status, MPIR_Request ** req, int hst_vci, int rmt_vci)
{
...
MPL_STATIC_INLINE_PREFIX int MPIDI_irecv_safe(void *buf,
                                              ...
                                              MPIR_Comm * comm,
                                              int context_offset, MPIDI_av_entry_t * av,
                                              MPIR_Request ** req, int hst_vci, int rmt_vci)
{
    int mpi_errno = MPI_SUCCESS, cs_acq = 0;
    ...
    if (!cs_acq) {
        ...
        *(req) = NULL;
        MPIDI_workq_vci_progress_unsafe();
        mpi_errno =
            MPIDI_irecv_unsafe(buf, count, datatype, rank, tag, comm, context_offset, av, req, hst_vci, rmt_vci);
    }
                                       ...
                                       MPIR_Request ** request)
{
    ...
    if (!MPIDI_COMM_VCI_HASH(comm).tag_par) {
        ...
        hst_vci = MPIDI_vci_get_host_recvs(comm, rank, tag);
        rmt_vci = MPIDI_vci_get_remote_recvs(comm, rank, tag);
    }

    if (unlikely(rank == MPI_PROC_NULL)) {
        MPIR_Request *rreq = MPID_Request_create_safe(MPIR_REQUEST_KIND__RECV, hst_vci);
                                        ...
                                        MPIR_Request ** request)
{
    ...
    av = MPIDIU_comm_rank_to_av(comm, rank);
    mpi_errno =
        MPIDI_irecv_safe(buf, count, datatype, rank, tag, comm, context_offset, av, request, hst_vci, rmt_vci);
